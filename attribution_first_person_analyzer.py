...
record_file_path = "/home/nick/temp/first_person_stats/attributions.csv"
with open(record_file_path) as record_file:
    attributions_df = pandas.read_csv(record_file, index_col=0)

for x, y in attributions_df.groupby("has_first_person"):
    if x == True:
        records_with_first_person = y
...
def summarize_by_participant(df,
                             total_mentions=False,
                             mention_tallies=False,
                             mention_proportions=False,
                             per_annotation=False):
    ...
    summarize_analysis(
        analyze_records(df),
        # mention_tallies=mention_tallies,
...
if __name__ == "__main__":
    summarize_by_participant(attributions_df)
    print()
    print("filter: has_first_person == True")
    summarize_by_participant(records_with_first_person)
